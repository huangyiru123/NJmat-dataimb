...
# import ase,catboost,catboost_info,chardet,docutils,dotenv,gensim,graphviz,joblib,matminer,monty,numpy, paramiko, pymatgen,rdkit,requests,seaborn,tensorflow,tensorflow_estimator,xgboost,yaml
# 数据不平衡  SMOTE 5-24  5-27
import dialog_data_imbalance_classified_data_two_RandomForest
import dialog_data_imbalance_classified_data_two_ExtraTree
import dialog_data_imbalance_classified_data_two_GaussianProcess
import dialog_data_imbalance_classified_data_two_SVM
import dialog_data_imbalance_continuous_data_DecisionTree
import dialog_data_imbalance_classified_data_two_Adaboost
import dialog_data_imbalance_classified_data_two_xgboost
...
class mywindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        # 数据不平衡 hyr  5-27  不平衡 SMOTE  过采样 子采样
        # 数据不平衡 随机森林 5-24
        self.actionRandom_forest.triggered.connect(self.show_dialog_data_imbalance_classified_data_two_RandomForest)
        self.actionExtra_tree.triggered.connect(self.show_dialog_data_imbalance_classified_data_two_ExtraTree)
        # self.actionGaussian_process.triggered.connect(self.show_dialog_data_imbalance_classified_data_two_GaussianProcess)
        self.actionDecision_tree.triggered.connect(self.show_data_imbalance_dialog_classified_data_two_DecisionTree)
        self.actionSVM_2.triggered.connect(self.show_data_imbalance_dialog_classified_data_two_SVM)
    ...
    # 数据不平衡 过采样 子采样 SMOTE 5-24 5-27--------------------------------------------------下面是引掉的部分
    def Data_imbalance_Classifieddata_Two_RandomForest(self):
        try:

            path = self.opt.save_path + "/Machine Learning Modeling/Algorithms/Classified data(two)/Data imbalance/Random Forest"
            csvname = self.opt.save_path + "/Preprocessing/Rfe feature selection" + "/data_rfe.csv"
            if os.path.exists(path):
                shutil.rmtree(path)
            os.makedirs(path)
            ...
            str1,str2,str3=dataML.data_imbalance_randomforest_Classifier(int(self.num_1),int(self.num_2),int(self.num_3),int(self.num_4),
                                               int(self.num_5),int(self.num_6), path,csvname)
            ...
            print(e)
    def show_dialog_data_imbalance_classified_data_two_RandomForest(self):
        def give(a,b,c,d,e,f):
            self.num_1=a
            self.num_2 = b
            self.num_3 = c
            ...
            self.num_5 = e
            self.num_6 = f
            self.Data_imbalance_Classifieddata_Two_RandomForest()

        if self.enter_training_test_set_path_state == True:
            if self.rfe_feature_selection_state == True:
                self.di = QtWidgets.QDialog()
                d = dialog_data_imbalance_classified_data_two_RandomForest.Ui_Dialog()
                d.setupUi(self.di)
                self.di.show()
    ...
    # 多余树 extratree
    def Data_imbalance_Classifieddata_Two_ExtraTree(self):
        try:

            path = self.opt.save_path + "/Machine Learning Modeling/Algorithms/Classified data(two)/Data imbalance/Extra Tree"
            csvname = self.opt.save_path + "/Preprocessing/Rfe feature selection" + "/data_rfe.csv"
            if os.path.exists(path):
                shutil.rmtree(path)
            os.makedirs(path)
            ...
            str1,str2,str3=dataML.data_imbalance_extratrees_classifier(int(self.num_1),int(self.num_2),int(self.num_3),int(self.num_4),
                                               float(self.num_5),int(self.num_6),path,csvname)
            ...
            print(e)
    def show_dialog_data_imbalance_classified_data_two_ExtraTree(self):
        def give(a,b,c,d,e,f):
            self.num_1=a
        ...
        if self.enter_training_test_set_path_state == True:
            if self.rfe_feature_selection_state == True:
                self.di = QtWidgets.QDialog()
                d = dialog_data_imbalance_classified_data_two_ExtraTree.Ui_Dialog()
                d.setupUi(self.di)
                self.di.show()
    ...
    def Data_imbalance_Classifieddata_Two_GaussianProcess(self):
        try:

            path = self.opt.save_path + "/Machine Learning Modeling/Algorithms/Classified data(two)/Data imbalance/Gaussian Process"
            csvname = self.opt.save_path + "/Preprocessing/Rfe feature selection" + "/data_rfe.csv"
            if os.path.exists(path):
                shutil.rmtree(path)
            os.makedirs(path)
            ...
            str1,str2,str3=dataML.data_imbalance_gaussianprocess_Classifier(float(self.num_1),float(self.num_2),int(self.num_3),int(self.num_4),
                                               int(self.num_5), path,csvname)
            ...
            print(e)
    def show_dialog_data_imbalance_classified_data_two_GaussianProcess(self):
        def give(a,b,c,d,e):
            self.num_1=a
            self.num_2 = b
            self.num_3 = c
            ...
            self.data_imbalance_Classifieddata_Two_GaussianProcess()
    ...
    # 决策树
    # 机器学习建模-模型选择-二分类-Decision Tree                         Classified data_Two_Decision Tree
    def Data_imbalance_Classifieddata_Two_DecisionTree(self):
        try:
            path = self.opt.save_path + "/Machine Learning Modeling/Algorithms/Classified data(two)//Data imbalance/Decision Tree"
            csvname = self.opt.save_path + "/Preprocessing/Rfe feature selection" + "/data_rfe.csv"
            if os.path.exists(path):
                shutil.rmtree(path)
            os.makedirs(path)
            ...
            str1, str2, str3 = dataML.data_imbalance_DecisionTree_classifier(float(self.num_1), int(self.num_2), float(self.num_3),
                                                              int(self.num_4),
                                                              int(self.num_5), path, csvname)
    ...
    def show_data_imbalance_dialog_classified_data_two_DecisionTree(self):
        def give(a, b, c, d, e):
            if a[1:-1] == "gini" or a[:] == "gini":  # 'gini', 'entropy'
                self.num_5 = 0.1
            elif a[1:-1] == "entropy" or a[:] == "entropy":
            ...
            if c[1:-1] == "auto" or a[:] == "auto":  # 'auto', 'sqrt', 'log2', None
                ...
                QMessageBox.information(self, 'Hint', 'max_features parameter error!',
                                        QMessageBox.Ok | QMessageBox.Close,
                                        QMessageBox.Close)
            self.num_4 = d
            self.num_5 = e
            self.Data_imbalance_Classifieddata_Two_DecisionTree()
    ...
    # SVM
    def Data_imbalance_Classifieddata_Two_SVM(self):
        try:

            path = self.opt.save_path + "/Machine Learning Modeling/Algorithms/Classified data(two)//Data imbalance/SVM"
            csvname = self.opt.save_path + "/Preprocessing/Rfe feature selection" + "/data_rfe.csv"
            if os.path.exists(path):
                shutil.rmtree(path)
            os.makedirs(path)
            ...
            str1, str4, str2, str3 = dataML.data_imbalance_SVM_classifier(int(self.num_1), float(self.num_2), int(self.num_3),
                                                           float(self.num_4),
                                                           float(self.num_5), path, csvname)
    ...
    def show_data_imbalance_dialog_classified_data_two_SVM(self):
        def give(a, b, c, d, e):
            self.num_1 = a
            ...
            if e == 'True':
                ...
                QMessageBox.information(self, 'Hint', 'Probability parameter error!',
                                        QMessageBox.Ok | QMessageBox.Close,
                                        QMessageBox.Close)

            self.Data_imbalance_Classifieddata_Two_SVM()
    ...
    #             ['sub_env/bin/python', 'sub_program.py', '--num_1', str(self.num_1), '--num_2', str(self.num_2),
    #              '--num_3', str(self.num_3), '--num_4', str(self.num_4), '--num_5', str(self.num_5), '--num_6',
    #              str(self.num_6),
    #              '--path', path, '--csvname', csvname],
    ...
    #
    #         if self.opt.if_open:
    #             for file in ['RandomForest_test_ROC.png', 'RandomForest_test_CM.png',
    #                          'RandomForest_train_ROC.png', 'RandomForest_train_CM.png']:
    #                 os.startfile(os.path.join(path, file.replace("/", "\\")))
    #
    #     except Exception as e:
    #         print(e)
    #
    ...
    #         self.Data_imbalance_Classifieddata_Two_RandomForest()
    #
    #     if self.enter_training_test_set_path_state:
    #         if self.rfe_feature_selection_state:
    #             self.di = QtWidgets.QDialog()
    #             d = dialog_data_imbalance_classified_data_two_RandomForest.Ui_Dialog()
    #             d.setupUi(self.di)
    #             self.di.show()
    #             d.buttonBox.accepted.connect(lambda: give(d.lineEdit.text(), d.lineEdit_2.text(), d.lineEdit_3.text(),
    #                                                       d.lineEdit_4.text(), d.lineEdit_5.text(),
    #                                                       d.lineEdit_6.text()))
    #             d.buttonBox.rejected.connect(self.di.close)
    #         else:
    #             QMessageBox.information(self, 'Hint', 'Do "RFE feature selection"!', QMessageBox.Ok | QMessageBox.Close,
